...
\begin{enumerate}
    ...
    \item \textbf{Coeficiente de variação do comprimento de frase} (\texttt{sent\_burst}): razão entre o desvio padrão e a média do comprimento de frases ($CV = \sigma / \mu$). Esta medida de dispersão relativa normaliza a variabilidade pela tendência central, permitindo comparações entre textos com diferentes médias de comprimento. É uma estatística adimensional amplamente utilizada para comparar variabilidade entre distribuições com escalas distintas.
    ...
    \item \textbf{Variabilidade da distribuição de caracteres} (\texttt{char\_entropy}): medida de dispersão na distribuição de frequências de caracteres, calculada pela fórmula de Shannon $H = -\sum p(c) \log_2 p(c)$, onde $p(c)$ é a frequência relativa de cada caractere. Esta medida quantifica a variabilidade: alta entropia indica distribuição mais uniforme (maior dispersão entre caracteres); baixa entropia indica concentração em poucos caracteres (menor dispersão). Estatisticamente, funciona como uma medida de dispersão análoga ao desvio padrão, mas aplicada a distribuições de frequência categórica.
