...
\begin{enumerate}
    \item \textbf{Simplicidade excessiva:} funções triangulares e agregação por média são escolhas básicas. Funções Gaussianas, trapezoidais ou em forma de sino, combinadas com operadores de agregação mais sofisticados (Choquet, Sugeno), poderiam melhorar o desempenho.
...
\begin{itemize}
    ...
    \item \textbf{Fuzzy tem menor variância:} $\sigma_{\text{fuzzy}} = 0.0004$ vs $\sigma_{\text{LDA}} = 0.0017$ vs $\sigma_{\text{logística}} = 0.0014$. Isto sugere que fuzzy é menos sensível à composição específica das partições.

    \item \textbf{Custo de oportunidade favorável:} a perda de 7,9\% em AUC (de 97\% para 89\%) é compensada por explicabilidade total, uma troca que pode ser valiosa em contextos sensíveis.
\end{itemize}
...
As funções de pertinência determinadas por quantis (33\%, 50\%, 66\%) capturam bem a estrutura dos dados, mas \textbf{não otimizam diretamente para separação de classes}. Abordagens futuras poderiam aprender limiares discriminativos (e.g., via algoritmos genéticos, otimização por enxame de partículas) para maximizar AUC.
...
\begin{enumerate}
    ...
    \item A abordagem orientada por dados (quantis) elimina a necessidade de definição manual de parâmetros, tornando o método escalável.
...
\begin{enumerate}
    ...
    \item Implementar aprendizado de parâmetros fuzzy via otimização
    \item Aplicar sistemas fuzzy tipo-2 para modelar incerteza nas próprias funções de pertinência
    \item Validar em domínios textuais específicos e com múltiplos modelos de LLM
\end{enumerate}
