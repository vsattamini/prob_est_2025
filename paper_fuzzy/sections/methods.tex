...
Utilizamos 10 características estilométricas extraídas pelo módulo \texttt{src/features.py}: (1) \texttt{sent\_mean} -- comprimento médio de frase em palavras; (2) \texttt{sent\_std} -- desvio padrão do comprimento de frase; (3) \texttt{sent\_burst} -- burstiness ($\sigma/\mu$); (4) \texttt{ttr} -- relação tipo-token ($V/N$); (5) \texttt{herdan\_c} -- C de Herdan ($\log V / \log N$); (6) \texttt{hapax\_prop} -- proporção de hapax legomena; (7) \texttt{char\_entropy} -- entropia de Shannon sobre caracteres; (8) \texttt{func\_word\_ratio} -- proporção de palavras funcionais (86 palavras em português); (9) \texttt{first\_person\_ratio} -- proporção de pronomes de primeira pessoa; e (10) \texttt{bigram\_repeat\_ratio} -- proporção de bigramas únicos repetidos.
